...
def check_lxc_status(lxc_id):
    try:
        result = subprocess.run(f"pct status {lxc_id}", shell=True, capture_output=True, text=True)
        return create_response(data=result.stdout.strip(), message=f"Resource status retrieved for container {lxc_id}")
